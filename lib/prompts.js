...
INDUSTRY READ (critical — do NOT default to tech/PM vocabulary)
- The user's industry is whatever their dump shows. Detect it from their actual language.
- Mirror their industry's native vocabulary, metrics, titles, and rituals:
  * Consulting / MBB / Big Four — engagements, partner staffing, case teams, decks, utilization, BD pipeline, manager / sr. consultant / EM / partner ladder.
  * Banking / Finance / IB — deal flow, MD coverage, modeling, pitch books, comps, accretion/dilution, group head, associate / VP / director ladder.
  * Marketing / Brand / Growth — campaigns, brand lift, attribution, channel mix, creative reviews, MQL/SQL, CMO cadence.
  * Healthcare / Clinical — panel size, case load, M&M conferences, patient outcomes, RVUs, attending / resident / fellow structure.
  * Education / Academia — cohorts, mastery, parent-teacher conferences, curriculum design, tenure track, dean / chair structure.
  * Legal — matter management, partner sponsorship, billable hours, deal closes, associate / counsel / partner ladder.
  * Operations / GM / Strategy — P&L, headcount, process redesign, planning cycles, BU / portfolio terms.
  * Sales / BD — pipeline, quota, account expansion, deal cycles, RFP, AE / SE / SDR.
  * Government / Policy — programs, appropriations, stakeholder coalitions, briefings, GS-level / political ladder.
  * Tech / Product — sprints, PRs, OKRs, roadmaps, stakeholders, IC / EM / staff / principal. Use these ONLY if the dump explicitly uses them.
- A consultant's "stakeholder syncs" are not a PM's. A banker's "deals" are not a PM's "deals." A clinician's "case load" has nothing to do with a salesperson's "pipeline." Get the vocabulary right or the reflection fails.
...
OUTPUT
- When asked for JSON: return ONLY a single valid JSON object. Start your response with { and end with }. NO preamble text ("Here is the JSON:"). NO postscript ("Let me know if..."). NO markdown code fences (\`\`\`). NO commentary anywhere. Just the JSON.
...
const REFLECT_PROMPT = (dumpContent) => `Read this raw dump from a user. It's messy by design — could be project briefs, case engagements, deal flow, campaigns, patient panels, programs, billable matters, Slack threads, calendar exports, performance feedback, or any combination. The user's industry is whatever their dump shows.

INTERPRETATION DISCIPLINE
- Stay close to the dump. Do NOT extrapolate or invent. If a theme isn't strongly evidenced in the actual content, omit it. Fewer high-conviction reflections beat more speculative ones.
- Every "evidence" field must quote or closely paraphrase actual content from the dump — not an interpretation layered on top. If you find yourself reaching, drop the theme.
- If the dump is thin or ambiguous, return fewer themes / wins / moves. Don't pad to hit a count.
- Detect the user's industry from their dump and use ITS vocabulary in your reflections. A consultant's reflections should sound like consulting; a banker's like banking. Do not impose product/tech framing unless the dump is product/tech.
